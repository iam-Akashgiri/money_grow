...
class MISMain(models.Model):
    ...
    product = fields.Char("Product - HL/LAP/ HL BT/ LAP BT")
    sales_manager_name = fields.Char("Sales Manager Name")
    sales_manager_mob_no = fields.Char("Sales Manager Mobile No")
    customer_contact_no = fields.Char("Customer Contact No")
    customer_email = fields.Char("Customer Email")
    loan_amount = fields.Float("Loan Amount", tracking=1)
    mode = fields.Char("Mode - Chq/DD/Online")
    cheq_dd = fields.Char("Cheque/DD Number")
    cheq_dd_amount = fields.Char("Cheque/DD Amount")
    clearance_date = fields.Date("Clearance Date")
    cheq_dd_bank_name = fields.Char("Chq/DD Bank Name")
    payment_details = fields.Char("Payment Details", help="Payment details as per the finacle particulars details.")
    vendor_payment_id = fields.Many2one("vendor.payment.confirmation", "Vendor Payment", help="Vendor Payment confirmation ( Only for vendor use )")
    fsd_received = fields.Char("FSD Received (Y/N)")
    document_sharing_status = fields.Char("Document Sharing Status")
    rac_remarks = fields.Many2one("rac.remarks", "RAC Remarks")
    vendor_remarks = fields.Many2one("vendor.remarks", "Vendor Remarks")
    all_data_shared_with_anulom = fields.Date("All Data Shared With Anulom")
    em_creation_date = fields.Date("EM Creation Date")
    noi_receipt_received_date = fields.Date("NOI Receipt Received Date")
    receipt = fields.Many2many(comodel_name='ir.attachment', string='Receipt')
    index_2 = fields.Many2many(comodel_name='ir.attachment',string="Index II", relation='index')
    customer_email_status = fields.Binary("Customer Email Status")
    bank_email_status = fields.Binary("Bank Email Status")
    submitted_to_igr = fields.Date("Submitted To IGR")
    ...
    rf_paid = fields.Integer("RF Paid")
    DHC = fields.Integer("DHC")
    stamp_duty = fields.Integer("Stamp Duty")
    comments = fields.Char("Comments")
    freeze_all = fields.Boolean("Freeze All", copy=False)
    can_edit = fields.Boolean("can_edit", copy=False, compute='_can_edit')
    show_submit_button = fields.Boolean("Show Submit Button", cumpute="_compute_show_submit")
    show_set_to_draft_button = fields.Boolean("Show Set To Draft Button", cumpute="_compute_show_set_to_draft_button")
